...
{
    // 特殊情况处理
    if (x == 0 && y == 0) return 0; // 或NaN，取决于具体需求
    if (x == 0) {
    ...
    int sign = 1;
    int n = 0;
    while (fabs(term) > 1e-10 && n < 10) { // 限制项数和精度
        result += sign * term;
        n++;
        term = -power(t, 2 * n + 1) / (2 * n + 1);
        sign *= -1;
    }
    if (x > 0)
    {
    ...
    {
        return (my_sign(y) * M_PI - result); // sign(y)确保正确象限
    }
}
...
//double my_otheratan2(double y, double x)
//{
//    double theta;
//
//    // 处理 x = 0 的特殊情况
...
//        }
//    }
//    // 如果 y < 0 且 x > 0，则 theta 已经是第四象限的负值
//
//    return theta;
//}

// 辅助函数：计算tan(y)
double my_tan(double y)
{
    return func_sin(y) / func_cos(y);
}

//// 使用牛顿迭代法计算atan(x)
//double my_atan(double x) {
//    const double epsilon = 1e-10; // 迭代精度
//    double y = x; // 初始猜测值，对于atan(x)，x本身是一个不错的起点
//    while (1) {
//        double tan_y = my_tan(y);
//        double next_y = y - (tan_y - x) / (1 + tan_y * tan_y); // 牛顿迭代公式
//        if (fabs(next_y - y) < epsilon) { // 检查收敛性
//            break;
//        }
//        y = next_y;
//    }
