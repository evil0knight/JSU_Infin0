...
{  
    ...
    distance = 2 * asin(func_sqrt(func_pow(func_sin(a / 2), 2) + func_cos(rad_latitude1) * func_cos(rad_latitude2) * func_pow(func_sin(b / 2), 2)));   // google maps 里面实现的算法
...
float get_two_points_azimuth2 (float latitude1, float longitude1, float latitude2, float longitude2)
{
    latitude1 = ANGLE_TO_RAD(latitude1);
    latitude2 = ANGLE_TO_RAD(latitude2);
    longitude1 = ANGLE_TO_RAD(longitude1);
    ...
    float x = sinf(longitude2 - longitude1) * cosf(latitude2);
    float y = cosf(latitude1) * sinf(latitude2) - sinf(latitude1) * cosf(latitude2) * cosf(longitude2 - longitude1);
    float angle = RAD_TO_ANGLE(atan2f(x, y));
    return ((0 < angle) ? angle : (angle + 360));
//    return angle;
}
//-------------------------------------------------------------------------------------------------------------------
