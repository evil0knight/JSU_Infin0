...
void pwm_init (pwm_channel_enum pwmch, uint32 freq, uint32 duty)
{
    ...
    //死区设置
    uint32 dead_time_ticks = (DEAD_TIME_NS * 1e-9) * CMU_CLK_FREQ;
//    g_atomConfig.dutyCycle = max(dead_time_ticks, (uint32)((uint64)duty * g_atomConfig.period / PWM_DUTY_MAX));

    switch(atom_channel->atom)
    {
        case 0: g_atomConfig.dutyCycle = max(dead_time_ticks, (uint32)((uint64)duty * g_atomConfig.period / PWM_DUTY_MAX)); break;
        case 1: g_atomConfig.dutyCycle = max(dead_time_ticks, (uint32)((uint64)duty * g_atomConfig.period / PWM_DUTY_MAX)); break;
        case 2: g_atomConfig.dutyCycle = max(dead_time_ticks, (uint32)((uint64)duty * g_atomConfig.period / PWM_DUTY_MAX)); break;
        case 3: g_atomConfig.dutyCycle = max(dead_time_ticks, (uint32)((uint64)duty * g_atomConfig.period / PWM_DUTY_MAX)); break;
        default:break;
    }
//    switch(atom_channel->atom)
//    {
