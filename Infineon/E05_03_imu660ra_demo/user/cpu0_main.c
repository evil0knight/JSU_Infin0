...
int core0_main(void)
{
    ...
    while(1)
    {
       if(imu660ra_init())
           ...
           break;
       if(imu660ra_gyro_x>100)gpio_toggle_level(LED1);                                                 // 翻转 LED 引脚输出电平 控制 LED 亮灭 初始化出错这个灯会闪的很慢
    }
    ...
    while (TRUE)
    {
        ...
        ips200_show_string              (0, 60, "imu660ra_gyro_x");
        ips200_show_float               (120, 60, imu660ra_gyro_x, 4, 4);
...
IFX_INTERRUPT(cc60_pit_ch0_isr, 0, CCU6_0_CH0_ISR_PRIORITY)
{
    ...
    pit_clear_flag(CCU60_CH0);  //去除定时标志位

    imu660ra_get_acc();                                                         // 获取 IMU660RA 的加速度测量数值
    imu660ra_get_gyro();                                                        // 获取 IMU660RA 的角速度测量数值
    if(imu660ra_gyro_x>100)gpio_toggle_level(LED1);
}
