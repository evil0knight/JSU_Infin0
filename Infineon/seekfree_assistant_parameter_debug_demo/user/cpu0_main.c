    ...
    while (TRUE)
    {
        ...
        for(uint8_t i = 0; i < SEEKFREE_ASSISTANT_SET_PARAMETR_COUNT; i++)
        {
            ...
            if(seekfree_assistant_parameter_update_flag[i])
            {
                ...
                //angle = seekfree_assistant_parameter[i];
                pwm_ph1 = seekfree_assistant_parameter[i];
                //pwm_ph1= seekfree_assistant_parameter[i];
                //wireless_uart_send_buffer((uint8 *)send_str,strlen(send_str));
            }
            //seekfree_assistant_oscilloscope_data.data[0] = high;
            //seekfree_assistant_oscilloscope_data.data[1] = angle;
            //seekfree_assistant_oscilloscope_send(&seekfree_assistant_oscilloscope_data);
        }
        //计算
        servo_control_table(high, angle, &pwm_ph1, &pwm_ph4);
        //限幅防卡死
        ...
        pwm_set_duty(SERVO_2, SERVO2_MID + pwm_ph4);
        pwm_set_duty(SERVO_3, SERVO3_MID + pwm_ph1);
