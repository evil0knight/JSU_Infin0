...
{
    pid2_flexible.run_speed=0;//速度初始化
    pid2_flexible.mechanical_neutral_point=-1;//机械中值初始化
     ...
     * */
    PID_Init(&pid2_flexible.jiaosudu_left,0.4,0.06,0.03,1500,3000);//角速度环参数初始化,机械中值-3
    PID_Clear(&pid2_flexible.jiaosudu_left);//清空角速度环pid的历史数据
    PID_Init(&pid2_flexible.jiaosudu_right,0.4,0.06,0.03,1500,3000);//角速度环参数初始化,机械中值-3
...
void pid1_walk_init(void)//pid行走
{
     ...
     * float maxOut             输出限幅                         300
     * */
     PID_Init(&pid1_walk.zhuanxiang,0.5,0.004,0.1,500,1000);//转向环参数初始化
     PID_SetErrLpfRatio(&pid1_walk.zhuanxiang,0.7);//角速度低通滤波，不然太抖
