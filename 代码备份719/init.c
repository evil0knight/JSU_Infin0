...
void mode_switch(int mode_choice){
    if(mode_choice==walk){
        ...
        PID_Init(&pid1_walk.sudu_left,0.14,0.0003,0,30,30);//速度环参数初始化
        PID_Init(&pid1_walk.sudu_right,0.14,0.0003,0,30,30);//速度环参数初始化0.06,
    }
        ...
        PID_Init(&pid1_walk.jiaodu_left,-50,0,-350,2000,3000);//角度环参数初始化,机械中值-3
        PID_Init(&pid1_walk.jiaodu_right,-50,0,-350,2000,3000);//角度环参数初始化,机械中值-3
        PID_Init(&pid1_walk.sudu_left,0.09,0.003,0,15,20);//速度环参数初始化
        PID_Init(&pid1_walk.sudu_right,0.09,0.003,0,15,20);//速度环参数初始化
    }
}
void pid1_walk_init(void)//pid行走
{
    ...
    PID_Init(&pid1_walk.sudu_left,0.09,0.003,0,15,20);//速度环参数初始化
    PID_Init(&pid1_walk.sudu_right,0.09,0.003,0,15,20);//速度环参数初始化0.06,
    /* CascadePID *pid          pid结构体                      pid1_walk站立pd的转向环
     * float p                  比例项参数                       0
     * float i                  积分项参数                       0
     * float d                  微分项参数                       0
