...
IFX_INTERRUPT(cc60_pit_ch1_isr, 0, CCU6_0_CH1_ISR_PRIORITY)
{
    ...
    // 调用跟随式横滚角补偿函数
    follow_pitch_compensation(1);
    wheel_obstacle_height_control(1);
        ...
        case jump:{//模式0，站立,并级pid
            if(ms20==20){
             ...
             if(ms10==10){
                 ...
                 pid1_walk.turn_value=turn_value;
                 /*mechanical_neutral_point机械中值，init.h中*/
                 position_pid(&pid1_walk.jiaodu_left,pid1_walk.mechanical_neutral_point,angle_roll);//角度环，反馈的是x轴角度
                 position_pid(&pid1_walk.zhuanxiang,pid1_walk.turn_value+140,-imu660ra_gyro_z);//转向环左，反馈的是z轴角速度
              }
             if(ms5==5){
                 ms5=0;
