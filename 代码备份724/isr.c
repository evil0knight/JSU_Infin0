...
int delayt=0;
int delayt_flag=0;
//uint32_t system_time_ms = 0; // 全局毫秒计数器，用于计时
unsigned char need_acceleration_control = 1;  // 是否需要调用加速控制
...
IFX_INTERRUPT(cc60_pit_ch1_isr, 0, CCU6_0_CH1_ISR_PRIORITY)
{
    ...
    ++ms10;
    ++ms20;
    //++system_time_ms; // 每1ms自增1，提供全局时间计数
    ...
    // 智能调用渐进加速控制
    if (need_acceleration_control) {
        need_acceleration_control = gradual_acceleration_control();
    }
...
IFX_INTERRUPT(cc61_pit_ch0_isr, 0, CCU6_1_CH0_ISR_PRIORITY)
{
    ...
    get_turn_value(100,0.8,250,0.1);//本来应该放在核1中的
